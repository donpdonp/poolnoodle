...
def price(x: Decimal, y: Decimal, x_add: Decimal, fee: Decimal, A: int):
    # constant sum -> D = (x+y)
    # constant product -> (D/2)^2 =  (x*y)
    out = get_dy(0, 1, x_add, [x, y], A, fee)
    print(f"price: get_dy = in = {x_add} out = {out} x/y {x_add/out} y/x {out/x_add}")
    return x_add / out

# https://github.com/curvefi/curve-contract/blob/master/contracts/pools/steth/StableSwapSTETH.vy

def get_dy(
    i: int, j: int, dx: Decimal, xp: List[Decimal], amp: int, fee: Decimal
) -> Decimal:
    x: Decimal = xp[i] + dx
